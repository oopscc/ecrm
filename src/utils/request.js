...
export default function request(url, options) {
    ...
    else if (newOptions.method === 'formdata') {
        newOptions.method = 'POST';
    }
    return fetch(url, newOptions)
        .then(checkStatus)
        .then((response) => {
            if (newOptions.method === 'DELETE' || response.result === 204) {
...
export function c_upload(props) {
    const { dispatch } = store;
    const { action, file, filename = 'fileData' } = props;
